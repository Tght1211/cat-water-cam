...
1. **小头用 `sklearn.linear_model.LogisticRegression(class_weight="balanced")`**：自带 L2 正则 + 类别平衡，
   小数据上更稳，30MB 依赖在 30G 磁盘里可忽略；省去手写训练/测试与过拟合调参。
2. **本轮只交付第一阶段**：`ClipJudge` 抽象 + `VLMClipJudge` + 邮件/计数闸到「真喝水」+ 计数口径翻转 +
   `ai_label_enabled` 默认开。第二阶段（本地 VideoMAE 裁判）等真攒够约 100~200 段标注素材再做——
   现在没数据下锅，先把判定权威切到 VLM、让标注工厂开跑。本文件第二阶段章节保留为下一轮的设计依据。
